...
class Daftar extends React.Component {
  state = {
    ...
    // eslint-disable-next-line
    regexPassword: new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*_\-])(?=.{8,})"),
    ...
    axios(request)
      .then(response => {
        ...
        swal({
          ...
          text: "Silahkan lakukan verifikasi akun anda dengan mengunggah foto KTP di halaman profil",
          icon: "success"
        })
            ...
            <Col>
              ...
              <Form onSubmit={event => event.preventDefault()}>
                ...
                <Form.Group className="masuk-password">
                  ...
                  <Form.Control
                    className={
                      this.state.regexPassword.test(this.state.password) || this.state.password === ""
                      ? ""
                      : "masuk-email"
                    }
                  ...
                  />
                  {
                    this.state.regexPassword.test(this.state.password) || this.state.password === ""
                    ? <div></div>
                    : <Form.Text className="masuk-emailtext">Panjang password minimal 8 karakter dan memuat minimal 1 huruf besar, 1 huruf kecil, 1 angka, dan 1 karakter spesial</Form.Text>
                  }
                  <div
                ...
                <Form.Group className="masuk-password">
                  ...
                  <Form.Control
                    ...
                    type={this.state.showKonfirmasiPassword ? "text" : "password"}
                    onChange={event => this.setState({konfirmasiPassword: event.target.value})}
                  />
                  {
                    this.state.konfirmasiPassword === this.state.password
                    ? <div></div>
                    : <Form.Text className="masuk-emailtext">Konfirmasi password anda tidak sesuai</Form.Text>
                  }
                  <div
                    className="masuk-showpassword"
                    onClick={() => {
                  ...
                  >
                    {this.state.showKonfirmasiPassword ? <FaEye /> : <FaEyeSlash />}
                  </div>
                </Form.Group>
                <Form.Group className="masuk-password">
                  ...
                  <Form.Control
                    type="text"
                    onChange={event => this.setState({namaDepan: event.target.value})}
                  />
                </Form.Group>
                <Form.Group className="masuk-password">
                  ...
                  <Form.Control
                    type="text"
                    onChange={event => this.setState({namaBelakang: event.target.value})}
                  />
                </Form.Group>
                <Form.Group className="masuk-password">
                  ...
                  <Form.Control
                    className={
                      this.state.regexNomorTelepon.test(this.state.nomorTelepon) || this.state.nomorTelepon === ""
                      ? ""
                      : "masuk-email"
                    }
                    type="text"
                    onChange={event => this.setState({nomorTelepon: event.target.value})}
                  />
                  {
                    this.state.regexNomorTelepon.test(this.state.nomorTelepon) || this.state.nomorTelepon === ""
                    ? <div></div>
                    : <Form.Text className="masuk-emailtext">Nomor telepon harus berupa 10-15 digit angka</Form.Text>
                  }
                </Form.Group>
                {
                  this.state.loading
